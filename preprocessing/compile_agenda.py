        ...
        if citation_finder:
            ...
            for event in record['timetable']:
                ...
                if 'fr_citation' in event:
                    try:
                        candidate_frdocs = citation_finder(event['fr_citation'])
                    except ValueError:
                        print(f'Warning: Could not parse fr_citation "{event["fr_citation"]}"')
                        continue

                    if len(candidate_frdocs) == 1:
                        event['frdoc_number'] = candidate_frdocs[0]

                    elif len(candidate_frdocs) > 1:
                        candidates_df = load_info_df(candidate_frdocs,fields=['frdoc_number','regulation_id_numbers','cfr_references','agencies','agencies_short','title'])
                        ...
                        if len(df) == 1:
                            ...
                            if rin_agencies:
                                ...
                                df = candidates_df[candidates_df['agency_name_match'] | candidates_df['agency_acronym_match']]
                                if len(df) == 1:
                                    event['frdoc_number'] = df['frdoc_number'].values[0]
